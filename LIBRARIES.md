...
| `<MapDrop image="..." zones={[...]} />` | none (custom, Pointer Events) | n/a | Drag-and-drop map labeling: chips from a shuffled tray onto percent-positioned zones over an image. Learn (instant feedback) and Test (check at end) modes, tap-to-place fallback for trackpads, reveal, reset, fullscreen toggle (native API with fixed-overlay fallback), per-activity localStorage best score, confetti on perfect. Zones accept an optional `short` pin label (full name in the tray and while dragging, abbreviation once placed). Built for the 13-colonies map lab. |
