...
If the embedded player below shows "Video unavailable" you can swap the YouTube URL in the guide source for any video you'd like (look for the `<VideoPlayer url="...">` line near the balcony scene section). Many official R&J productions disable embedding, so a search link below is provided as a fallback.

<VideoPlayer url="https://www.youtube.com/watch?v=YeqYZ8tOArc" />

[Search YouTube for full balcony scene productions →](https://www.youtube.com/results?search_query=romeo+juliet+balcony+scene+full)
...
<Callout type="tip" title="Listen to the soliloquy">
Free public-domain readings of the full balcony scene are available on LibriVox. Try [LibriVox's Romeo and Juliet collection](https://librivox.org/search?title=romeo+and+juliet&author=Shakespeare) for several full-cast versions.
</Callout>
