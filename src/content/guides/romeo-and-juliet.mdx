...
<NivoSankey
  data={{
    nodes: [
      ...
      { id: "Tybalt's pride" },
      { id: "Mercutio's death" },
      { id: "Romeo kills Tybalt" },
      { id: 'Banishment' },
      { id: "Friar's risky plan" },
      { id: 'Plague delays letter' },
      { id: "Romeo's suicide" },
      { id: "Paris dies at tomb" },
      { id: "Juliet's suicide" },
      { id: 'Reconciliation' }
    ],
    links: [
      { source: 'Ancient feud', target: "Tybalt's pride", value: 30 },
      { source: "Tybalt's pride", target: "Mercutio's death", value: 30 },
      { source: "Mercutio's death", target: "Romeo kills Tybalt", value: 30 },
      { source: "Romeo kills Tybalt", target: 'Banishment', value: 30 },
      { source: 'Banishment', target: "Friar's risky plan", value: 30 },
      { source: "Friar's risky plan", target: 'Plague delays letter', value: 30 },
      { source: 'Plague delays letter', target: "Romeo's suicide", value: 25 },
      { source: "Friar's risky plan", target: "Paris dies at tomb", value: 5 },
      { source: "Romeo's suicide", target: "Juliet's suicide", value: 25 },
      { source: "Paris dies at tomb", target: 'Reconciliation', value: 5 },
      { source: "Juliet's suicide", target: 'Reconciliation', value: 25 }
    ]
  }}
/>
...
This is the most famous scene in English drama. Watch it once, then we'll memorize it.

<VideoPlayer url="https://www.youtube.com/watch?v=R0n_oa1KVl4" />

<Callout type="note" title="Production note">
This is the Zeffirelli 1968 cut, the version most schools show. The Luhrmann 1996 version transposes it to a swimming pool in modern Verona Beach, California. Both are faithful to the text.
</Callout>

### Scrolling the soliloquy

<Scrollytelling
  graphic={`<div style='font-family: Fraunces, serif; font-size: 22px; line-height: 1.7; color: #fafaf9; background: linear-gradient(135deg, #18181b, #0a0a0a); padding: 32px; border-radius: 16px; max-width: 100%; box-shadow: inset 0 0 60px rgba(185,28,28,0.15);' id='balcony-text'>
  <div data-line='1' style='transition: opacity 600ms;'>But, soft! what light through yonder window breaks?</div>
  <div data-line='2' style='opacity: 0.3; transition: opacity 600ms;'>It is the east, and Juliet is the sun.</div>
  <div data-line='3' style='opacity: 0.3; transition: opacity 600ms;'>Arise, fair sun, and kill the envious moon,</div>
  <div data-line='4' style='opacity: 0.3; transition: opacity 600ms;'>Who is already sick and pale with grief...</div>
</div>`}
  steps={[
  ...
  onStep={`(stepId) => {
    ...
    stage.querySelectorAll('[data-line]').forEach((el) => {
      ...
      el.style.opacity = n <= target ? '1' : '0.3';
    });
  }`}
/>
