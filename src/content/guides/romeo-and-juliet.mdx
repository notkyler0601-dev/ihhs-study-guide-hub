...
import Pronounce from '../../components/Pronounce.astro';
...
<MapLibreView
  center={[10.999, 45.443]}
  zoom={14.5}
  height="420px"
  markers={[
    { lng: 10.998, lat: 45.443, label: "Casa di Giulietta (the famous balcony, 14th c. courtyard)", color: "#b91c1c" },
    { lng: 11.001, lat: 45.444, label: "Piazza delle Erbe (the marketplace where Act 1 brawl happens)", color: "#b91c1c" },
    { lng: 10.992, lat: 45.439, label: "Tomba di Giulietta (the tomb, in a former monastery)", color: "#7f1d1d" },
    { lng: 11.003, lat: 45.444, label: "Casa di Romeo (a private 13th c. fortified house)", color: "#7f1d1d" },
    { lng: 10.996, lat: 45.439, label: "San Fermo Maggiore (the church Romeo and Juliet's marriage is associated with)", color: "#52525b" }
  ]}
/>
...
<Callout type="tip" title="Three productions worth watching">
Each link opens a YouTube search so you always get a fresh, working clip (production uploads come and go).
...
### Scrolling the soliloquy

<Scrollytelling
  graphicHeight="60vh"
  graphic={`<div style='position: relative; width: 100%; height: 100%; min-height: 60vh; font-family: Fraunces, serif; color: #fafaf9; background: radial-gradient(circle at 75% 25%, #1c1917 0%, #0a0a0a 70%); border-radius: 16px; padding: 40px 32px; box-shadow: inset 0 0 80px rgba(185,28,28,0.18); overflow: hidden; display: flex; flex-direction: column; justify-content: center;' id='balcony-text'>
  <svg id='balcony-moon' style='position: absolute; top: 24px; right: 28px; width: 64px; height: 64px; transition: opacity 800ms, filter 800ms; filter: drop-shadow(0 0 18px rgba(255,255,200,0.4));' viewBox='0 0 64 64'>
  ...
  <svg id='balcony-window' style='position: absolute; top: 110px; right: 36px; width: 48px; height: 64px; transition: opacity 800ms; opacity: 0;' viewBox='0 0 48 64'>
    <rect x='4' y='4' width='40' height='56' rx='6' fill='#fef3c7' opacity='0.85'/>
    <line x1='24' y1='4' x2='24' y2='60' stroke='#7f1d1d' stroke-width='2'/>
    <line x1='4' y1='32' x2='44' y2='32' stroke='#7f1d1d' stroke-width='2'/>
  </svg>
  <div style='font-size: 20px; line-height: 1.85; max-width: 78%;'>
    <div data-line='1' style='transition: opacity 600ms;'>But, soft! what light through yonder window breaks?</div>
    <div data-line='2' style='opacity: 0.25; transition: opacity 600ms;'>It is the east, and Juliet is the sun.</div>
    <div data-line='3' style='opacity: 0.25; transition: opacity 600ms;'>Arise, fair sun, and kill the envious moon,</div>
    <div data-line='4' style='opacity: 0.25; transition: opacity 600ms;'>Who is already sick and pale with grief...</div>
  </div>
  <div style='position: absolute; bottom: 16px; left: 32px; font-size: 11px; letter-spacing: 0.18em; text-transform: uppercase; color: rgba(250,250,249,0.5);'>Act 2, Scene 2</div>
</div>`}
  steps={[
    { id: 'open', body: '<h3>“But, soft!”</h3><p>The line opens with a beat-break: <em>BUT</em>, <em>SOFT</em>, two stressed syllables back-to-back. Shakespeare uses this when a character is suddenly arrested by what they see. Romeo is mid-thought when Juliet appears.</p>' },
    { id: 'sun', body: '<h3>“Juliet is the sun.”</h3><p>Romeo crowns Juliet a heavenly body. The metaphor sets up the entire light-vs-dark imagery system that runs through the rest of the play. Juliet is light; the night and the moon are her enemies.</p>' },
    { id: 'moon', body: '<h3>The envious moon</h3><p>The moon is the goddess Diana, virginity. Romeo wants Juliet to “kill” it, that is, give up chastity. This is a young man\'s seduction speech dressed up as cosmology.</p>' },
    { id: 'sick', body: '<h3>Foreshadowing</h3><p>“Sick and pale with grief.” The play is full of <em>death</em> imagery in moments of love. Romeo cannot praise Juliet without invoking what will undo her.</p>' }
  ]}
  onStep={`(stepId) => {
    const stage = document.getElementById('balcony-text');
    if (!stage) return;
    const map = { open: 1, sun: 2, moon: 3, sick: 4 };
    const target = map[stepId] || 1;
    stage.querySelectorAll('[data-line]').forEach((el) => {
      const n = Number(el.getAttribute('data-line'));
      el.style.opacity = n <= target ? '1' : '0.25';
    });
    const win = document.getElementById('balcony-window');
    const moon = document.getElementById('balcony-moon');
    if (win) win.style.opacity = target >= 1 ? '1' : '0';
    if (moon) {
      if (target >= 3) { moon.style.opacity = '0.4'; moon.style.filter = 'drop-shadow(0 0 8px rgba(120,113,108,0.3)) grayscale(0.7)'; }
      else { moon.style.opacity = '1'; moon.style.filter = 'drop-shadow(0 0 18px rgba(255,255,200,0.4))'; }
    }
  }`}
/>
...
Use the four modes to lock the first eight lines into long-term memory. Most students can hit "first letter" mode within 20 minutes and "cold recall" mode by the next day.

<Memorize
  title="Romeo's balcony soliloquy (lines 2-9)"
...
<Callout type="tip" title="Memorization order">
1. **Read** mode three times out loud, slowly, with feeling.
2. **Cloze** mode: fill in the blanks aloud. Click any blank to peek.
3. **First letter** mode: now you have only initials. Speak the full line.
4. **Cold recall** mode: reveal one line at a time. If you got the next line right before clicking, you're done.
</Callout>

### Hear it spoken

<Pronounce src="https://archive.org/download/romeo_juliet_librivox/romeoandjuliet_07_shakespeare.mp3" label="Public-domain LibriVox reading of Act 2, Scene 2 (the full balcony scene)" />
